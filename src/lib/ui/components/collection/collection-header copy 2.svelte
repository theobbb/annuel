...
<div class="grid-12">
	...
	<div class="col-span-6">
		<div class="flex justify-end gap-8 whitespace-nowrap">
			{#each tabs as { name, param, length }}
				<a
					class={['', current_tab === param ? 'underline' : '']}
...
<div class="border-b-2- col-span-6- col-start-7- mt-24">
	<div
		class="justify-between- flex gap-8"
		role="list"
		onmouseleave={() => (pop_visible = false)}
		onblur={() => (pop_visible = false)}
	>
		<a href="">Tous les programmes</a>
...
<div
	class={[
		'pointer-events-none fixed top-0 right-0 left-0 z-20 h-60 bg-background mix-blend-difference',
...
<div
	class={[
		'pointer-events-none fixed top-60 right-0 left-0 z-20',
		pop_visible ? '' : '-translate-y-2 opacity-0',
		'transition ease-in-out'
	]}
>
	{#if hovered}
		<div
			class="grid-12 min-h-[50svh]- grid-rows-[1fr_auto] items-end gap-y-6! bg-black px-8 py-8 text-balance text-background"
		>
