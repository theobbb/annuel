...
{#if year?.poster}
	<div
		class={[
			...
			playing && 'opacity-0 transition-opacity delay-200 duration-500'
		]}
	>
		{#each posterConfigs as [left, top, rotate], i}
			<div
                ...
                "
				class="ease absolute top-0 left-0 w-max transition-transform duration-500 will-change-transform"
			>
				<Image
					collection="years"
					record_id={year.id}
